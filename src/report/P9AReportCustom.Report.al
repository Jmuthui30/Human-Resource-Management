report 52070 "P9A Report Custom"
{
    ApplicationArea = All;
    DefaultLayout = RDLC;
    RDLCLayout = './src/report_layout/P9AReportCustom.rdlc';
    UseRequestPage = true;
    Caption = 'P9A Report Custom';
    dataset
    {
        dataitem(Employee; Employee)
        {
            ...
            begin
                ...
                CUser := UserId;
                if not UserSetup.Get(CUser) then
                    Error('Your User account has not been linked to a Staff Number; Kindly contact ICT');

                if UserSetup.Get(CUser) then
