report 52102 "Payroll Regulatory Summary"
{
    ApplicationArea = All;
    DefaultLayout = RDLC;
    RDLCLayout = './src/report_layout/PayrollRegulatorySummary.rdl';
    Caption = 'Payroll Regulatory Summary';
    dataset
    {
        dataitem(AssignmentMatrix; "Assignment Matrix")
        {
            DataItemTableView = sorting("Payroll Period", Type, Code) where("Tax Relief" = const(false), "Non-Cash Benefit" = const(false), "Employee Type" = filter(<> Trustee));
